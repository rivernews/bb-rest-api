...
resource "aws_launch_configuration" "ecs-launch-configuration" {
  name                 = "${var.project_name}-ecs-launch-configuration"
#   name_prefix                 = "${var.project_name}-"
  ...
  # TODO: associate appropriate security group
...
resource "aws_autoscaling_group" "ecs-autoscaling-group" {
  ...
  vpc_zone_identifier = ["${var.subnet_ids[0]}", "${var.subnet_ids[1]}"] # when say vpc id, it's really talking about subnet
...
resource "aws_ecs_cluster" "test-ecs-cluster" {
  name = "${var.project_name}-myecscluster"
}
